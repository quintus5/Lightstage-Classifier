bit = 65535;
%% noisefree image
myFolder = 'G:\work\matlab\basler\classtest\plate3\100ms0db';
...
%% single illum
myFolder = 'G:\work\matlab\basler\classtest\plate3\step';
filePattern = fullfile(myFolder, '*.tiff');
jpegFiles = dir(filePattern);
figure;
for i = 1:numpos
    ...
    title(['VAR: ',num2str(std2(sim{i})^2),' mean ',num2str(mean2(sim{i})),' SNR ',...
        num2str(10*log10(mean2(sim{i})/std2(sim{i})^2))]);pause(0.01);
    var1(i) = std2(sim{i})^2;
end
%%
c = [8,7,6,5,4,3,2,1];
...
myFolder = 'G:\work\matlab\basler\classtest\plate3\100ms0dbm';
filePattern = fullfile(myFolder, '*.tiff');
jpegFiles = dir(filePattern);
for j = 1:numcolor
    figure;
...
%         axis off;colorbar;colormap bone;%caxis([-20,0]);
        title(['VAR: ',num2str(std2(dem{k})^2)]);pause(0.01);
